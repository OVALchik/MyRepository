...
namespace OOPlaba2Test
{
    ...
    public class Idustry
    {
        ...
        public void IndustryConstructor_Error()
        {
            try
            {
                var industry = new Industry("TestInd", null);
                Assert.Fail();
            }
        ...
        public void GetCountPiples()
        {
            ...
            var productionListSt = new List<PrimaryProduction>{ new PrimaryProduction(TypeMaterial.Aluminum, TypeProduction.Plate, "Лист AL-4", 50, 70, 5, 3, 1000, 250m)};

            var pipleListPr = new List<string> { "Захарова Е.Ф.", "Лубинин П.Я.", "Нестеров Н.П.", "Захарова Е.Ф.", "Лубинин П.Я.", "Нестеров Н.П. "};
            var productionListPr = new List<SecondaryProduction>{new SecondaryProduction(TypeMaterial.Aluminum, TypeProduction.Plate, "Заготовка-AL5", 50, 50, 12, 5,500, 350m)};

            var pipleListAs = new List<string> { "Захарова Е.Ф.", "Лубинин П.Я."};
            var productionListAs = new List<FinalProduction>{new FinalProduction("Станок измерительный", 100, 200, 160, 15, 1100, 5000m)};

            var st = new StorageDepartment(machineList, "Заготовительный цех№1", pipleListSt, new List<Production>(productionListSt));
            var pd = new ProcessingDepartment("Обрабатывающий цех№1", pipleListPr, new List<Production>(productionListPr));
            var ad = new AssemblyDepartment("Сборочно-монтажный цех№1", pipleListAs, new List<Production>(productionListAs));
